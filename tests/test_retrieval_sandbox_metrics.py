...
def test_reciprocal_rank_fusion_scores_ordering():
    ...
    scores = reciprocal_rank_fusion(rank_lists=rank_lists, retriever_weights=weights, k=60)
    assert scores["a"] > 0
...
def test_apply_source_boost_changes_priority():
    ...
    boosted = apply_source_boost(scores=scores, source_stance=source_stance, boosts=boosts)
