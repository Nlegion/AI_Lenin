...
class PhilosophicalTextProcessor:
    def __init__(self):
        ...
        try:
            ...
            subprocess.run(
                [sys.executable, "-m", "spacy", "download", "ru_core_news_lg"]
            )
            self.nlp = spacy.load("ru_core_news_lg")
        ...
        try:
            ...
            print(
                "Попробуйте установить вручную: pip install transformers sentence-transformers"
            )
            raise

        # Философский тезаурус
        self.philosophy_terms = self.load_philosophy_terms()
        self.lenin_specific_terms = [
            "партийность",
            "эмпириомонизм",
            "богостроительство",
            "махист",
    ...
    def restore_notebook_structure(self, text):
        ...
        for i, line in enumerate(lines):
            ...
            source_match = re.search(
                r"(\b(Гегель|Маркс|Фейербах|Аристотель|Энгельс|Плеханов)\b)", line
            )
            if source_match:
    ...
    def restore_letter_structure(self, text):
        ...
        text = re.sub(
            r"(Уважаемый\s+[А-Я][а-я]+!|Дорогой\s+[А-Я][а-я]+!)",
            r"\n[ОБРАЩЕНИЕ: \1]\n",
            text,
        )
        text = re.sub(
            r"(С\s+товарищеским\s+приветом[,\s]*|С\s+коммунистическим\s+приветом[,\s]*)",
            r"\n[ПОДПИСЬ: \1]\n",
            text,
        )
        return text
    ...
    def restore_critique_structure(self, text):
        ...
        text = re.sub(
            r"(Он\s+утверждает,? что|По\s+его\s+мнению,?)",
            r"\n[ТЕЗИС ОППОНЕНТА]: \1",
            text,
        )
        return text
