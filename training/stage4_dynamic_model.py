...
def load_template():
    ...
    try:
        ...
        logger.info(
            f"Шаблон мировоззрения загружен, концептов: {len(template['unique_patterns'])}"
        )
        return template
...
def initialize_model():
    ...
    try:
        model = SentenceTransformer(
            "paraphrase-multilingual-mpnet-base-v2", device="cuda", truncate_dim=512
        )
...
def process_volume(model, index, concept_names, volume_path, volume_id):
    ...
    try:
        ...
        if not os.path.exists(volume_path):
            ...
            return {"volume_id": volume_id, "error": f"File not found: {volume_path}"}
        ...
        for i, section in enumerate(sections):
            ...
            if exclude_reasons:
                ...
                if i == 0:
                    ...
                    logger.debug(
                        f"Исключен раздел 0: Причины: {exclude_reasons}\nОбразец: '{sample}'"
                    )
                continue
        ...
        distances, indices = index.search(
            section_embeddings.astype(np.float32), TOP_CONCEPTS_PER_SECTION
        )
        ...
        for i, (D, I) in enumerate(zip(distances, indices)):
            ...
            for idx, score in zip(I, D):
                ...
                top_concepts.append({"concept": concept, "similarity": float(score)})
        ...
        avg_transformation = float(
            np.mean([s["transformation_index"] for s in volume_results])
        )
        ...
        return {"volume_id": volume_id, "error": str(e)}
...
def analyze_evolution(volumes_data):
    ...
    for i in range(1, len(evolution)):
        delta = (
            evolution[i]["avg_transformation"] - evolution[i - 1]["avg_transformation"]
        )
...
def main():
    ...
    for volume_id, volume_path in enumerate(
        tqdm(volume_paths, desc="Обработка томов"), start=1
    ):
        if not os.path.exists(volume_path):
        ...
        result = process_volume(
            model, index, concept_names, volume_path, volume_id
        )  # Исправлено здесь
    ...
    worldview_model = {
        "metadata": {
            **template["metadata"],
            ...
            "total_sections": sum(
                len(v["sections"]) for v in volumes_data if "sections" in v
            ),
    ...
    logger.info(
        f"Точек изменения мировоззрения выявлено: {len(evolution_analysis['turning_points'])}"
    )
