...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("ontology_builder.log"), logging.StreamHandler()],
)
...
class OntologyBuilder:
    ...
    def extract_key_concepts(self, text):
        ...
        if concepts:
            ...
            logger.info(
                f"Обработка текста завершена за {time.time() - start_time:.2f} сек."
            )
        ...
        for i in range(len(filtered_concepts)):
            ...
            for j in range(i + 1, len(filtered_concepts)):
                if similarity_matrix[i, j] > 0.6:
                    ...
                    self.concept_relations[concept1][concept2] = float(
                        similarity_matrix[i, j]
                    )
                    relations_count += 1

        logger.info(
            f"Добавлено {relations_count} отношений за {time.time() - start_time:.2f} сек."
        )
