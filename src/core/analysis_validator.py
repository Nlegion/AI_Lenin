...
class AnalysisValidator:
    ...
    def validate_analysis(self, analysis: str, news_title: str = "") -> Dict:
        ...
        if len(analysis.strip()) < self.min_length:
            validation_result["reasons"].append(
                f"Слишком короткий анализ ({len(analysis)} символов)"
            )
            return validation_result
        ...
        for topic in self.forbidden_topics:
            if topic in analysis_lower:
                validation_result["reasons"].append(
                    f"Обнаружена запрещенная тема: '{topic}'"
                )
                break

        # СМЯГЧЕННАЯ проверка релевантной терминологии
        marxist_terms_count = sum(
            1 for term in self.marxist_terms if term in analysis.lower()
        )

        # Только предупреждение, но не блокировка
        if marxist_terms_count == 0:
            validation_result["reasons"].append(
                "Отсутствует марксистско-ленинская терминология"
            )
        ...
        if found_errors:
            validation_result["reasons"].append(
                f"Обнаружены грамматические ошибки: {', '.join(found_errors[:3])}"
            )
        ...
        if (
            len(validation_result["reasons"]) <= 3
        ):  # Увеличили допустимое количество причин
            validation_result["is_valid"] = True
            validation_result["score"] = self._calculate_score(
                analysis, marxist_terms_count, validation_result["has_quotes"]
            )
    ...
    def _calculate_score(
        self, analysis: str, marxist_terms_count: int, has_quotes: bool
    ) -> float:
        """Рассчитывает оценку качества анализа"""
        length_score = min(1.0, len(analysis) / 200)  # Нормализуем длину
        terms_score = min(
            1.0, marxist_terms_count / 3
        )  # Нормализуем количество терминов
        ...
        return (
            0.4 * length_score + 0.3 * terms_score + 0.2 * sentence_score + quote_score
        )
