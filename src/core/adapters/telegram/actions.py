...
class TelegramActions:
    ...
    async def about_me(self):
        try:
            ...
            self.logger.error(
                "Error fetching bot info", error=str(e), function="about_me"
            )
            return None
    ...
    async def get_bot_groups(self):
        try:
                ...
                for update in updates["result"]:
                    ...
                    if message and "chat" in message:
                        ...
                        if chat["type"] in ["group", "supergroup"]:
                            ...
                            username = chat.get(
                                "username", chat.get("first_name", "NoUsername")
                            )
                            groups.add((chat["id"], username))
                if groups:
                    self.logger.info(
                        "Bot chats", groups=groups, function="get_bot_groups"
                    )
                    return list(groups)
            ...
            self.logger.error(
                "Error fetching bot groups", error=str(e), function="get_bot_groups"
            )
            return []

    async def send_message(
        self, chat_id: int, text: str, user_id: int = None, **kwargs
    ):
        if user_id:
        ...
        return await self.service.send_message(
            chat_id=chat_id, text=escaped_text, parse_mode="MarkdownV2", **kwargs
        )
