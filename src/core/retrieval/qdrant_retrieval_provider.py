...
class QdrantRetrievalProvider:
    ...
    def _dense_search(
        ...
        vector = (
            dense_vector
            or self.model.encode([query_text], normalize_embeddings=True)[0].tolist()
        )
    ...
    def retrieve_by_stance(
        ...
        *,
        ...
        if shared_vector is None and queries:
            shared_vector = self.model.encode([queries[0]], normalize_embeddings=True)[
                0
            ].tolist()
    ...
    def _prepare_queries(self, query_text: str) -> list[str]:
        base = (
            rewrite_query_to_philosophical_register(query_text)
    ...
    @staticmethod
    def _judge_scores(
        query_text: str, candidates: list[RetrievalCandidate]
    ) -> dict[str, float]:
        terms = {
            token
            for token in re.findall(r"[a-zA-Zа-яА-ЯёЁ]+", query_text.lower())
            if token
        }
        ...
        for candidate in candidates:
            payload_terms = {
                token
                for token in re.findall(r"[a-zA-Zа-яА-ЯёЁ]+", candidate.text.lower())
                if token
            }
            if not terms:
    ...
    def retrieve_with_trace(
        self, query_text: str, apply_judge: bool | None = None
    ) -> tuple[list[RetrievalCandidate], dict]:
        effective_apply_judge = (
            self.config.judge_enabled if apply_judge is None else apply_judge
        )
        ...
        for query in queries:
            dense = self._dense_search(
                query_text=query, retriever_name="dense", limit=self.config.top_k
            )
            ...
            dense_trace.extend(
                self._serialize_candidates(candidates=dense, query=query)
            )
            sparse_trace.extend(
                self._serialize_candidates(candidates=sparse, query=query)
            )
        ...
        chunk_to_stance = {
            candidate.chunk_id: candidate.stance_type for candidate in candidates
        }
        ...
        trace = {
            ...
            "onto": onto_trace,
            "merged_scores": {
                key: round(value, 6) for key, value in merged_scores.items()
            },
            "boosted_scores": {key: round(value, 6) for key, value in boosted.items()},
            "judge_scores": {
                key: round(value, 6) for key, value in judge_scores.items()
            },
            "final_scores": {
                key: round(value, 6) for key, value in final_scores.items()
            },
            "judge_enabled": effective_apply_judge,
        }
        return final, trace
