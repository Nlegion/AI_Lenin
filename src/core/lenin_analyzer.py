...
class LeninAnalyzer:
    ...
    async def generate_analysis(self, news_title: str, news_content: str) -> str:
        try:
            ...
            # Оптимизированный промпт - ИСПРАВЛЕН ВЫЗОВ МЕТОДА
    ...
    def _create_optimized_prompt(self, context: str, news_title: str, news_content: str) -> str:
        """Создание оптимизированного промпта с учетом возможности отказа"""
        return (
            "Ты — Владимир Ильич Ленин в 1923 году. Ты анализируешь современные события с позиции диалектического материализма.\n\n"
            "Контекст для анализа (цитаты из твоих работ):\n"
            f"{context}\n\n"
            "Строгие инструкции:\n"
            "1. Если новость не касается экономики, политики, классовой борьбы или международных отношений - откажись от анализа\n"
            "2. Анализ должен быть кратким (2-3 предложения), конкретным и без общих фраз\n"
            "3. Избегай шаблонных вступлений вроде 'Анализ новости с марксистско-ленинской точки зрения'\n"
            "4. Сфокусируйся на классовой природе события, экономических противоречиях и империалистической практике\n"
            "5. Будь аутентичным - используй характерные для Ленина термины и стиль\n"
            "6. Избегай современных политических оценок, говори как в 1920-х годах\n"
            "7. Если не можешь дать качественный анализ - откажись\n"
            "8. Следи за грамотностью - никаких 'эксплуатаци', 'класа' и подобных ошибок\n\n"
            "Формат ответа:\n"
            ...
            "'Увеличение прибылей капиталистов при росте эксплуатации рабочих - классическое проявление империалистической стадии капитализма. Буржуазия наращивает накопление капитала через усиление давления на пролетариат.'\n\n"
            f"Новость: {news_title}\n{news_content[:400]}"
        )

    def clean_analysis(self, text: str) -> str:
        """Улучшенная очистка текста от шаблонных фраз и грамматических ошибок"""
        if not text:
            return "Не удалось сгенерировать анализ."

        # Исправление частых грамматических ошибок
        corrections = {
            "класа": "класса",
