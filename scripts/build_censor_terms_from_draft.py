...
def _should_drop(term: str, category: str) -> str | None:
    ...
    if category == "WAR_OPERATIONAL" and term in LEISURE_TOPONYMS and term not in WAR_FRONTLINE_TOPONYMS:
    ...
    if (
        ...
        return "non_frontline_toponym"
    if category == "NATURAL_DISASTER" and not any(marker in term for marker in _NATURAL_KEEP_MARKERS):
    ...
    if term in AMBIGUITY_BANLIST:
        if category == "WAR_OPERATIONAL" and term in WAR_FRONTLINE_TOPONYMS:
            return None
        if category in SEED_TERMS and term in {t.casefold() for t in SEED_TERMS[category]}:
            return None
    ...
    if category == "AIRPORT" and term in {"борт", "пассажир"}:
        return "ambiguity_banlist"
    if category == "AUTO" and term in {"танк", "патриот", "газ", "автомат", "механик", "лада", "tank", "patriot"}:
        return "ambiguity_banlist"
    ...
    for category, seeds in SEED_TERMS.items():
        for term in seeds:
            ...
            if normalized in AMBIGUITY_BANLIST:
                drop_log.append((category, normalized, "seed_blocked_by_policy_banlist"))
                continue
