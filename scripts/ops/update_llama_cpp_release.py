...
def install_release(*, tag: str, llama_url: str, cudart_url: str, force: bool) -> Path:
    ...
    if not (release_root / "llama" / "llama-server.exe").exists():
        raise RuntimeError(
            f"llama-server.exe missing after install into {release_root}"
        )
...
def write_current_pointer(release_root: Path) -> None:
    ...
    try:
        ...
        (current / "RELEASE_PATH.txt").write_text(
            str(release_root.resolve()), encoding="utf-8"
        )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Update llama.cpp Windows CUDA release for GigaChat3."
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Re-download even if tag already installed.",
    )
    parser.add_argument(
        "--tag", default=None, help="Optional explicit tag, e.g. b10167."
    )
    ...
    release_root = install_release(
        tag=tag, llama_url=llama_url, cudart_url=cudart_url, force=bool(args.force)
    )
    write_current_pointer(release_root=release_root)
    locals_ = list_local_releases()
    logger.info(
        "Local releases (newest first): %s",
        ", ".join(path.name for path in locals_) or "(none)",
    )
    return 0
