...
def _run(command: list[str], cwd: Path) -> CommandResult:
    ...
    return CommandResult(
        command=command, exit_code=process.returncode, elapsed_seconds=elapsed
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run Alembic + pytest gates for a subplan."
    )
    parser.add_argument(
        "--pattern",
        type=str,
    ...
    if args.pattern:
        commands.append(
            [python_executable, "-m", "pytest", "tests", "-k", args.pattern, "-q"]
        )
    ...
    for result in results:
        ...
        print(
            f"- [{status}] {_format_command(result.command)} ({result.elapsed_seconds:.2f}s)"
        )
