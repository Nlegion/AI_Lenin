...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze short cleaned documents quality.")
    parser.add_argument(
        "--stats-json",
    ...
    parser.add_argument("--out-md", default=".cursor/artifacts/cleaning/short_docs_qc.md")
    parser.add_argument("--out-json", default=".cursor/artifacts/cleaning/short_docs_qc.json")
    ...
    short_docs = [row for row in payload["stats"] if int(row["output_lines"]) < args.max_lines]
    ...
    out_json_path.write_text(
        json.dumps({"short_docs_total": len(findings), "findings": findings}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
