...
def _emit_rate_block(report: list[str], *, title: str, rows: list[dict[str, Any]]) -> None:
...
def main() -> int:
    ...
    intentional_override_rows = [
        ...
        if "unknown_topic_low_signal_allow_forward" in [str(c) for c in (r.get("reason_codes") or [])]
        or "override:unknown_topic_forward_trusted_source"
        in [str(c) for c in (r.get("reason_codes") or [])]
    ]
    unexpected_conflict_rows = [r for r in l1_mismatch_rows if r not in intentional_override_rows]
    ...
    _emit_rate_block(report, title="## Control Decision Rates (All Rows)", rows=control_rows)
    _emit_rate_block(report, title="## Control Decision Rates (Unique news_id)", rows=control_unique)

    l1_counts = Counter(
        f"{str(r.get('l1_decision'))}->{str(r.get('decision'))}" for r in l1_mismatch_rows
    )
    report.extend(["", "## L1 To Final Mismatches", f"- mismatch_rows: {len(l1_mismatch_rows)}"])
    ...
    if args.compare_jsonl:
        ...
        compare_control = [r for r in compare_rows if str(r.get("dataset") or "live") == "control"]
        compare_control_unique = _first_unique_by_news_id(rows=compare_control)
        compare_live = [r for r in compare_rows if str(r.get("dataset") or "live") != "control"]
        report.extend(["", "## Compare Baseline", f"- compare_jsonl: {args.compare_jsonl}"])
        _emit_rate_block(report, title="### Current Live (Unique N/A)", rows=live_rows)
        _emit_rate_block(report, title="### Compare Live (Unique N/A)", rows=compare_live)
        _emit_rate_block(report, title="### Current Control Unique", rows=control_unique)
        _emit_rate_block(report, title="### Compare Control Unique", rows=compare_control_unique)
        report.extend(
            [
                "",
