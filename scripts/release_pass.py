...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if run_anti:
            _run([python, "scripts/evaluate_anti_cliche.py"], cwd=repo)
        ...
        if check_delta:
            ...
            if not baseline.is_file():
                ...
                base_fail = int(base_payload.get("failed", base_payload.get("failures", 0)) or 0)
                cur_fail = int(cur_payload.get("failed", cur_payload.get("failures", 0)) or 0)
