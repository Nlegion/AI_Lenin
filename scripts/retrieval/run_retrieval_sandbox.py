...
from src.core.vector.sandbox_metrics import (  # noqa: E402
    apply_source_boost,
    recall_at_k,
...
def _ontology_search(
    query_text: str, ontology_rows: list[dict[str, str]], limit: int
) -> list[str]:
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-json", default=".cursor/artifacts/sandbox/retrieval_sandbox_results.json"
    )
    parser.add_argument(
        "--out-md", default=".cursor/artifacts/sandbox/retrieval_sandbox_summary.md"
    )
    ...
    for row in eval_rows:
        ...
        onto_source_ids = _ontology_search(
            query_text=query_text, ontology_rows=ontology_rows, limit=config.top_k
        )
        ...
        for mode in config.modes:
            ...
            if mode == "hyde_hybrid":
                ...
                hyde_dense = dense_model.encode(
                    [hyde_query], normalize_embeddings=True
                )[0].tolist()
            ...
            ranked = [
                ...
                for source_id, _ in sorted(
                    boosted.items(), key=lambda item: item[1], reverse=True
                )
            ]
    ...
    for mode in config.modes:
        mode_recall = recall_at_k(
            predictions=mode_predictions[mode], positives=positives, k=5
        )
    ...
    best_mode = (
        max(results.items(), key=lambda item: item[1]["recall_at_5"])[0]
        if results
        else "none"
    )
    payload = {
