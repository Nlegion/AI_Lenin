...
def main() -> int:
    ...
    parser.add_argument(
        "--output-json", default=".cursor/artifacts/evaluation/rag_quality_metrics.json"
    )
    parser.add_argument(
        "--output-md", default=".cursor/artifacts/evaluation/rag_quality_summary.md"
    )
    parser.add_argument("--max-queries", type=int, default=40)
    args = parser.parse_args()

    provider = build_provider(
        config_path=(REPO_ROOT / args.retrieval_config).resolve(),
        base_dir=REPO_ROOT,
    )
    if provider is None:
        raise RuntimeError(
            f"retrieval provider unavailable (missing or disabled config): {args.retrieval_config}"
        )
    eval_rows = _read_eval(path=(REPO_ROOT / args.eval_dataset).resolve())[
        : args.max_queries
    ]
