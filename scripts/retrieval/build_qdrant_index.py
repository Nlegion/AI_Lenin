...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build Qdrant local index from chunks."
    )
    parser.add_argument("--config", default="config/qdrant_ingestion.yaml")
    parser.add_argument(
        "--chunks-tsv", default=".cursor/artifacts/chunks/chunk_dataset_v2.tsv"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--stats-json", default=".cursor/artifacts/qdrant/ingestion_stats_giga_v1.json"
    )
    parser.add_argument(
        "--summary-md", default=".cursor/artifacts/qdrant/ingestion_summary_giga_v1.md"
    )
    ...
    hw = hardware_report(
        resolved_device=resolved, fallback_to_cpu=config.fallback_to_cpu
    )
