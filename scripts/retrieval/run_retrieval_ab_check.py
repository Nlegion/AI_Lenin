...
def main() -> int:
    ...
    parser.add_argument(
        "--queries", default=None, help="Optional text file with one query per line."
    )
    parser.add_argument(
        "--out-json", default=".cursor/artifacts/retrieval/retrieval_ab_summary.json"
    )
    parser.add_argument(
        "--out-md", default=".cursor/artifacts/retrieval/retrieval_ab_summary.md"
    )
    ...
    queries = _load_queries(
        path=(REPO_ROOT / args.queries).resolve() if args.queries else None
    )
    ...
    payload = {
        ...
        "parity_shared_ratio_avg": statistics.mean(parity_values)
        if parity_values
        else None,
        "trace": trace,
    }
