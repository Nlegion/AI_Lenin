...
from src.core.embeddings.benchmark import (  # noqa: E402
    BenchmarkResult,
    choose_best_model,
    compute_recall_at_k,
)
...
def _build_documents(
    corpus_root: Path, registry_rows: list[dict[str, str]]
...
def main() -> int:
    ...
    parser.add_argument(
        "--eval-dataset", default=".cursor/artifacts/eval/embedding_eval.tsv"
    )
    parser.add_argument(
        "--source-registry", default=".cursor/artifacts/registries/source_registry.tsv"
    )
    parser.add_argument("--corpus-root", default="data/books")
    parser.add_argument("--config", default="config/embedding_benchmark.yaml")
    parser.add_argument(
        "--results-output",
        default=".cursor/artifacts/embeddings/benchmark_results.json",
    )
    parser.add_argument(
        "--decision-output",
        default=".cursor/artifacts/embeddings/embedding_selection.md",
    )
    parser.add_argument(
        "--max-models", type=int, default=0, help="Limit models for quick local run."
    )
    ...
    model_entries = [
        entry for entry in config.get("models", []) if bool(entry.get("enabled", True))
    ]
    ...
    documents, index_by_source = _build_documents(
        corpus_root=corpus_root, registry_rows=registry_rows
    )
    query_texts = [row["query"] for row in eval_rows]
    positives = [
        index_by_source[row["positive_source_id"]]
        for row in eval_rows
        if row["positive_source_id"] in index_by_source
    ]
