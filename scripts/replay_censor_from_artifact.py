...
async def _replay_rows(
    *,
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        content_hash, _ = compute_content_hash(title=title, body=body, url=str(row.get("url") or ""))
        ...
        changed = old_decision != result.decision or old_codes != list(result.reason_codes)
        out.append(
            {
                "line_no": idx,
                ...
                "body_missing_in_input": not bool(row.get("body") or row.get("content")),
...
def _write_dropped_duplicates_csv(path: Path, dropped_titles: dict[str, int]) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writeheader()
        for title, count in sorted(dropped_titles.items(), key=lambda item: item[1], reverse=True):
...
def main() -> int:
    ...
    parser.add_argument("--output-jsonl", required=True, help="Output JSONL with replay results")
    parser.add_argument("--output-csv", required=True, help="Output CSV for manual review")
    parser.add_argument(
        "--output-dropped-csv",
        default="",
    ...
    print(f"rows={len(replayed)} dropped_duplicates={dropped} changed={changed} unchanged={len(replayed)-changed}")
    return 0
