...
def _load_news_text(path_arg: str) -> NewsItem:
    ...
    return NewsItem(title=title, content=content, source="manual_input", url="stdin://", news_id="manual")
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Run local RAG dry-run with retrieval diagnostics.")
    parser.add_argument("--fixture", default=None)
    parser.add_argument("--news-id", default=None)
    parser.add_argument("--news-text", default=None, help="Path to file with title/content or '-' for stdin.")
    ...
    parser.add_argument("--audit-log", default=".cursor/artifacts/safety/dryrun_audit.jsonl")
    ...
    if args.bypass_safety:
        _safe_print("\n## WARNING")
        _safe_print("BYPASS_SAFETY enabled: NewsGate checks are skipped. Developer-only override.")
    ...
    generation_config = load_generation_config(path=(REPO_ROOT / args.generation_config).resolve())
    ...
    if qdrant_provider is not None and hasattr(qdrant_provider, "retrieve_with_trace"):
        ...
        result = provider.retrieve_context(query_text=enhanced_query, author_filter="Ленин")
        context = result.context
        trace = {"query_variants": [enhanced_query], "dense": [], "sparse": [], "onto": []}
    elif args.allow_legacy_fallback:
        fallback_used = True
        context = analyzer.context_orchestrator.build_context(enhanced_query=enhanced_query)
        trace = {"query_variants": [enhanced_query], "dense": [], "sparse": [], "onto": []}
    else:
        _render_section("SAFETY", "qdrant provider unavailable and legacy fallback disabled")
        return 4
    ...
    if args.verbose:
        _render_section("RETRIEVAL_DENSE", _top_n(trace.get("dense", []), limit=args.top_n))
        _render_section("RETRIEVAL_SPARSE", _top_n(trace.get("sparse", []), limit=args.top_n))
        _render_section("RETRIEVAL_ONTO", _top_n(trace.get("onto", []), limit=args.top_n))
        _render_section(
            "ARBITER",
            json.dumps(
                {
        ...
        json.dumps(
            {
                ...
                "config_hash_news_guard": _sha256_file((REPO_ROOT / args.news_guard_config).resolve()),
                "config_hash_generation": _sha256_file((REPO_ROOT / args.generation_config).resolve()),
                "corpus_manifest_hash": _sha256_file(
                    (REPO_ROOT / ".cursor/artifacts/cleaning/corpus_repro_manifest_v1.tsv").resolve()
                ),
                ...
                "hardware": hardware_report(
                    resolved_device=resolve_torch_device(preferred=retrieval_config.device),
                    fallback_to_cpu=bool(getattr(retrieval_config, "fallback_to_cpu", True)),
                ),
            },
            ensure_ascii=False,
            indent=2,
    ...
    if _recent_high_risk_count(path=audit_path) >= args.alert_threshold:
        _render_section("ALERT", "High-risk filter triggers exceeded threshold; notify administrator.")
    if fallback_used:
