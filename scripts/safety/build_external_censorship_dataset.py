...
def _fetch_url_status(url: str, retries: int = 3) -> dict[str, Any]:
    if urlparse(url).scheme not in {"http", "https"}:
        return {
            "ok": False,
            "status": 0,
            "error": f"unsupported URL scheme for {url!r}",
        }
    delay = 1.0
    last_error = ""
    for _ in range(retries):
        try:
            req = Request(
                url=url,
                method="GET",
                headers={"User-Agent": "ai-lenin-dataset-builder/1.0"},
            )
...
def build_manifest(
    *, sources_cfg: Path, output_manifest: Path, cache_manifest: Path
) -> dict[str, Any]:
    ...
    raise RuntimeError(
        "External sources unavailable and no last-known-good manifest found"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--sources-config", default="config/external_dataset_sources.yaml"
    )
    parser.add_argument(
        "--output-manifest", default=".cursor/artifacts/quality/dataset_manifest.json"
    )
    parser.add_argument(
        "--cache-manifest",
