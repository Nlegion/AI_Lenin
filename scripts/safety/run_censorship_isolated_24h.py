...
def _summary(rows: list[dict[str, Any]], *, elapsed_seconds: float) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        **_latency(rows),
        ...
        "l0_filtered_share": sum(
            1 for r in rows if "l0_filtered" in (r.get("reason_codes") or [])
        )
        / total,
    }
...
def main() -> int:
    ...
    control_batch_size = int(
        args.control_batch_size or cfg.get("control_batch_size", 0)
    )
    if control_batch_size > 0 and not control_rows:
        logger.error(
            "control_batch_size=%s but control set is empty/missing", control_batch_size
        )
        return 1
    ...
    metrics_path = out_dir / str(
        cfg.get("metrics_filename", "censorship_24h_latest.metrics.json")
    )
    notes_path = out_dir / str(
        cfg.get("notes_filename", "censorship_24h_latest.notes.md")
    )
    ...
    while time.time() < deadline:
        ...
        if (
            control_processed == 0
            and control_batch_size > 0
            and control_cursor >= len(control_rows)
        ):
    ...
    logger.info(
        "done rows=%s p95=%.2fms", len(rows), float(summary.get("p95_latency_ms", 0.0))
    )
    return 0
