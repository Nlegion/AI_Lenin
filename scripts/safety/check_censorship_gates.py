...
def _bootstrap_or_load_latency_baseline(
...
def main() -> int:
    ...
    hard_block_rate = _ratio(
        sum(1 for r in rows if _decision(r) == "hard_block"), total
    )
    ...
    if bool(latency_cfg.get("enabled", False)) and args.metrics_json:
        ...
        current = {
            ...
            "throughput_items_per_second": float(
                metrics.get("throughput_items_per_second", 0.0)
            ),
        ...
        p95_delta_max = float(
            latency_cfg.get(
                "p95_with_l3_delta_max_ms"
                if current["l3_used_share"] > 0
                else "p95_without_l3_delta_max_ms",
                40.0,
            )
        )
        p95_ratio_max = float(latency_cfg.get("p95_ratio_vs_baseline_max", 1.10))
        throughput_ratio_min = float(
            latency_cfg.get("throughput_min_ratio_vs_baseline", 0.8)
        )
        baseline_p95 = max(float(baseline.get("p95_latency_ms", 0.0)), 1e-6)
        baseline_thr = max(
            float(baseline.get("throughput_items_per_second", 0.0)), 1e-6
        )
        ...
        latency_summary = {
            ...
            "current_throughput_items_per_second": current[
