...
async def _run_batch(censor: PreRagCensor, items: list[dict[str, Any]]) -> dict[str, float]:
...
def _build_censor(*, base_dir: Path, mode: str) -> PreRagCensor:
    ...
    if mode == "l2_off":
        runtime = CensorRuntimeConfig(**{**runtime.__dict__, "l2_similarity_enabled": False})
    return PreRagCensor(
        safety_gate=gate,
        news_guard=guard,
        config=runtime,
...
def main() -> int:
    ...
    parser.add_argument("--out-json", default=".cursor/artifacts/quality/censorship_load_resilience_latest.json")
    ...
    baseline = asyncio.run(_run_batch(_build_censor(base_dir=base_dir, mode="baseline"), items))
    l2_off = asyncio.run(_run_batch(_build_censor(base_dir=base_dir, mode="l2_off"), items))
