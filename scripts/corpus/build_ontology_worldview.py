...
def _write_validation_sample(
    records: list[TaggedSource], output_path: Path, sample_size: int
) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    random.seed(42)
...
def _compute_iaa(validation_path: Path) -> float | None:
    ...
    filtered = [
        row
        for row in rows
        if row.get("annotator_a", "").strip() and row.get("annotator_b", "").strip()
    ]
    if not filtered:
        return None
    total = len(filtered)
    agreement = sum(
        1
        for row in filtered
        if row["annotator_a"].strip() == row["annotator_b"].strip()
    )
...
def _write_summary(
    ...
    contradiction_docs = sum(
        1 for record in tagged_records if record.contradiction_hits
    )
    lines = [
    ...
    if iaa_score is None:
        lines.append(
            "- IAA (annotator_a vs annotator_b): `N/A` (manual annotations not filled yet)"
        )
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build ontology tags and worldview graph."
    )
    parser.add_argument(
        "--registry",
    ...
    parser.add_argument(
        "--sample-size", type=int, default=25, help="Validation sample size."
    )
