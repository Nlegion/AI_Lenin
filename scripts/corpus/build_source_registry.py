...
def _write_summary_markdown(
    summary: dict[str, int], output_path: Path, corpus_root: Path
) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build source registry for corpus inventory."
    )
    parser.add_argument(
        "--corpus-root", default="data/books", help="Corpus root directory path."
    )
    parser.add_argument(
        "--rules-config",
    ...
    _write_summary_markdown(
        summary=summary, output_path=summary_output_path, corpus_root=corpus_root
    )
