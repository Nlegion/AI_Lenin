...
from scripts.lib._live_news_qa_24h_worker import process_live_item  # noqa: E402
from scripts.lib._live_news_qa_artifacts import (  # noqa: E402
    count_done_rows,
    rejected_paths,
    resolve_live_artifacts,
)
from scripts.lib._live_news_qa_censor import build_live_pre_rag_censor  # noqa: E402
...
from scripts.lib._quality_qa_runtime import rag_probe  # noqa: E402
from scripts.quality.run_live_news_qa_24h_cli import build_parser  # noqa: E402
...
async def async_main(args) -> int:
    ...
    logger.info(
        "24h_qa_start duration_h=%s poll_s=%s txt=%s",
        args.duration_hours,
        poll_s,
        artifacts.txt,
    )
    ...
    try:
        while time.monotonic() < deadline:
            ...
            try:
                ...
                logger.exception(
                    "fetch_failed cycle=%s error=%s", counters["cycles"], error
                )
                items = []
            logger.info(
            ...
            for item in items:
                ...
                processed_cycle += 1
                if int(args.max_per_cycle) > 0 and processed_cycle >= int(
                    args.max_per_cycle
                ):
                    break
