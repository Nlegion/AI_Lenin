...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--fetch-limit", type=int, default=0, help="Max RSS items per fetch (0=all)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--checkpoint", default=None, help="Resume from existing checkpoint JSONL."
    )
    parser.add_argument("--output-dir", default=".cursor/artifacts/quality")
    parser.add_argument(
        "--force", action="store_true", help="Wipe artifacts and start fresh."
    )
