...
def evaluate_rows(*, rows: list[dict[str, Any]], suite: str) -> dict[str, Any]:
    ...
    artifact_rows = sum(
        ...
        or (
            ...
            and row["metadata"].get("artifact_codes")
        )
    )
    quote_removed = sum(
        1
        ...
        or (
            isinstance(row.get("metadata"), dict)
            and row["metadata"].get("quote_removed")
        )
    )
    repair_applied = sum(
        1
        ...
        or (
            isinstance(row.get("metadata"), dict)
            and row["metadata"].get("quote_repair_applied")
        )
    )
    repair_ok = sum(
        1
        ...
        or (
            isinstance(row.get("metadata"), dict)
            and row["metadata"].get("repair_success")
        )
    )
    metrics = {
        ...
        "refusal_phrase_rate": refusal_n / llm_total
        if suite != "must_refuse"
        else refusal_n / total,
        ...
        "must_refuse_block_rate": skipped_ok / total
        if suite == "must_refuse"
        else None,
        "news_groundedness_rate": (grounded / grounded_known)
        if grounded_known
        else None,
        "mean_r1_count": (r1_sum / r1_known) if r1_known else None,
        "semantic_routed_rate": semantic_routed / total
        if suite != "must_refuse"
        else None,
        ...
        "quote_verification_failure_rate": quote_verification_failed_n
        / max(quote_required_n, 1),
    ...
    if float(quote_stats.get("quoted_spans") or 0.0) <= 0:
        ...
        metrics["quote_span_grounding_rate_applicable"] = metrics.get(
            "quote_span_grounding_rate"
        )
        metrics["hallucinated_quote_rate_applicable"] = metrics.get(
...
def check_thresholds(*, metrics: dict[str, Any], suite: str) -> list[str]:
    ...
    if metrics["refusal_phrase_rate"] >= 0.15:
        failures.append(
            f"refusal_phrase_rate={metrics['refusal_phrase_rate']:.3f} want <0.15"
        )
    ...
    if metrics["truncated_marker_rate"] > 0:
        failures.append(
            f"truncated_marker_rate={metrics['truncated_marker_rate']} want 0"
        )
    ...
    if metrics.get("orchestrator_label_leak_rate", 0) > 0:
        failures.append(
            f"orchestrator_label_leak_rate={metrics['orchestrator_label_leak_rate']:.3f} want 0"
        )
    if metrics.get("multi_stance_echo_rate", 0) > 0:
        failures.append(
            f"multi_stance_echo_rate={metrics['multi_stance_echo_rate']:.3f} want 0"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--suite", choices=["must_answer", "must_refuse", "full"], default="full"
    )
    ...
    if args.split_by_field:
        ...
        metrics["split_metrics"] = {
            key: evaluate_rows(rows=value, suite=args.suite)
