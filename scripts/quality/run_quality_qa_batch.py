...
from scripts.quality.run_quality_qa_batch_cli import build_parser  # noqa: E402
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    if args.guard_check_only:
        return run_guard_check(
            items=items, guard=guard, max_blocked_ratio=args.max_blocked_ratio
        )
    ...
    if args.pre_gate_only:
        ...
        logger.info(
            "wrote checkpoint=%s results=%s txt=%s",
    ...
    if is_llama_server_active(server_url=server_url):
        ...
        if not await owned_server.start_server():
            logger.error(
                "Failed to start llama-server for persona_model=%s",
                generation_config.persona_model,
            )
            return 3
        if not await _wait_for_health(
            ...
            logger.error(
                "llama-server did not become healthy within --start-wait=%s",
                args.start_wait,
            )
            return 3
    else:
        logger.error(
            "LLM not reachable at %s; pass --start-server or start llama-server",
            server_url,
        )
        return 3
    ...
    if analyzer.retrieval_provider is None and not args.allow_legacy_fallback:
        logger.error(
            "Qdrant/retrieval unavailable; re-run with --allow-legacy-fallback"
        )
    ...
    try:
        ...
        for index, item in enumerate(items, start=1):
            ...
            if should_skip_checkpoint_row(
                row=previous, input_hash=input_hash, force=args.force
            ):
                logger.info("[%s/%s] %s status=skip", index, total, item.id)
                continue
            if (
                previous is not None
                and str(previous.get("input_hash", "")) != input_hash
            ):
            ...
            answer_txt = (
                row.get("answer")
                or f"[ERROR: {row.get('error_type')}] {row.get('error')}"
            )
            with artifacts.txt.open("a", encoding="utf-8") as handle:
                handle.write(
                    format_txt_block(
