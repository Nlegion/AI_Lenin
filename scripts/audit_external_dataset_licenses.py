...
def main() -> int:
    ...
    parser.add_argument("--manifest", default=".cursor/artifacts/quality/dataset_manifest.json")
    parser.add_argument("--previous-manifest", default=".cursor/artifacts/quality/dataset_manifest.last_good.json")
    ...
    for row in payload.get("sources", []):
        ...
        if not bool(row.get("download_ok", False)):
            warnings.append(f"{source_id}: source unavailable, using last-known-good if present")
